...
if (isset($_GET['id'])) {
  if (!isset($_SESSION['email'])) {
    ...
    if (!$resume['resume']) {
      $_SESSION['message'] = "Please upload your contract first!";
      echo $_SESSION['message'];
      header("location: ../dashboard/myContract.php");
      exit();
    } else {
    ...
    $pdf->SetTitle('Talent Application Report');
    $pdf->SetSubject('Talent Application Data');
    $pdf->SetKeywords('TCPDF, Talent, Application');
    ...
    // Add headline
    $pdf->Cell(0, 10, 'Talent Application Report', 0, 1, 'C');
    ...
    Talent Title : {$row['jobtitle']}
    Talent-Category : {$industry['name']}
    Artist Name : {$row1['companyname']}
    State/Region : {$row2['name']}
...
";
    ...
    $pdf->Output('talent_application_report.pdf', 'I');
  }
  header('location: ../talentDetails.php?key=' . md5($id_jobpost) . '&id=' . $id_jobpost . '');
}
